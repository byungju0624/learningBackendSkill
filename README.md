...
>     1. 세션과 쿠키
>        1. 세션
>           - 사용자의 아이디 세션의 아이디 세션이 얼마동안 유효한지 별도의 세션의 정보를 세션이라는 데이터베이스에 저장한다.
>           - 세션이라는 데이터 베이스에 모든 세션의 정보를 보관하고 있으므로 신뢰할 수 있는 데이터가 존재한다.
>           - 단점 - 서버에서 시시각각 변하는 로그인하는 사용자에 대해서 세션의 정보를 보관하고 있으므로 서버에 상태가 있고, 클라이언트의 요청을 처리하기 위해서는 내부적으로 여러가지 네트워크 요청을 해야하므로 시간이 오래 걸릴 수 있다.
>        2. 쿠키
>           - 서버에서 보낼때도 쉽고 클라이언트에서 별도로 처리하지 않아도 브라우저에서 해줄 수 있으므로 간단하게 구현가능하다.
>           - HTTP Only를 사용하는 경우 안전하게 보안성을 높여서 사용할 수 있다.
>     2. JWT(JSON Web Token)
>     - JSOM을 이용해서 web token을 주고 받는 것
>     - Header, Payload, Signature로 이루어져 있다.
>     - Header: 사용하는 알고리즘과 타입에 대해서 명시가 돼 있다.
>     - Payload: 우리가 전송하고 주고받고 싶은 다양한 데이터들을 넣을 수 있다. 인코딩되어서 보안처리 되기 때문에 이해하기 어렵다.
>     - Signature: 우리가 입력한 Header와 Payload뿐만 아니라, 이것을 인코딩하기 위해서 사용하는 비밀키를 함께 인코딩한다. 이렇게 서버에서만 사용하는 시크릿과 함께 인코딩하므로써, 나중에 사용자가 페이로드에 있는 정보를 바꾼다면 시크니쳐를  
>        통해서 바뀌었는지 알수 있다.
>     - 서버에 스테이트가 없다.
>     3. bcrypt
>     - 패스워드를 안전하게 보관할 수 있도록 패스워드를 해싱하는 알고리즘
>     - 단방향
...
> 11. Database
>
> - 컴퓨터 파일 시스템에서 관련된 데이터끼리 모아놓은 것, 모아놓고 관리하고 읽고 쓰고 할 수 있는 것
> - DBMS(Database Management System): 데이터베이스를 조금 더 편리하게 관리해주는 시스템, 이런 데이터베이스를 이용해서 쉽게 데이터를 만들고 접근할 수 있도록 도와주는 것
> - SQL: 데이터베이스에 데이터를 만들고 저장하고 읽고 쓰고 할 수 있는 언어, RDBMS라고 한다. 관련있는 데이터끼리 각각의 테이블로 만들어 놓고 이들의 관계를 정의할 수 있다.
> - NoSQL: SQL이 아닌 모든 것들을 가리키는 조금 더 큰 카테고리
>
>   1. SQL
>      - 테이블과 로우 컬럼으로 구성돼 있다. 관련있는 데이터를 테이블 형식으로 관리한다.
>      - 각 행마다 특정한 데이터타입을 지정해줘야 한다. 여러가지 속성도 지정가능하다.
>      - 테이블과 컬럼과 로우 각각의 데이터 타입을 지정해주는 것을 스키마(Schema)라고 한다.
>      - 스키마를 통해서 Data integrity(데이터의 온전함,데이터의 진실성)를 보장할 수 있다. -> 스키마에서 정의한 다른 종류의 데이터나 다른 타입의 데이터를 넣을 수 없기 때문이다.
>      - Primary Key: 테이블에서 정말 고유한 식별자를 가릴 수 있는 ID, primary key로 데이터베이스가 검색의 최적화를 해주기 때문에 조금 더 빠르게 데이터를 찾을 수 있다.
>      - Foreign Key: 두 가지의 다른 테이블의 관계를 정해준다.
>      1. SQL Query
>         - SELECT: 테이블에서 어떤 행을 선택할 껀지 정할 수 있다.
>         - FROM: 어떤 테이블에서 가져올 것인지 정할 수 있다.
>         - WHERE: 조건을 걸 수 있다.
>   2. NoSQL
>      - 스키마가 없다.
>      - 관계형이 아니다. 저장하고자 하는 데이터를 여러개로 분산해서 다른서버에서 관리하기 용이하다.
>      - 용도에 따라서 사용가능하다.
>   3. ORM과 ODM
>      - ORM(Object Relational Mapping): 우리가 작성한 오브젝트를 자동으로 데이터베이스에 스키마를 만들고 자동으로 데이터를 저장하고 읽어와서 코드로 변환해주기도 한다.
>        - 비지니스 로직에 초점을 맞춰 개발할 수 있게 해준다.
>        - 반복을 줄여준다.
>        - 다른 SQL 데이터베이스를 사용해도 편하게 이용가능하다.
>        - 코드가 변경되면 마이그레이션이 지원된다.
>        - 상세한 쿼리를 할 수 없다.
>        - 복잡한 쿼리를 할 수 없다.
